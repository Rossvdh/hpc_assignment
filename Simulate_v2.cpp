...
int serial(int argc, char const *argv[]) {
	...
	// molfile_timestep_t* timesteps = new molfile_timestep_t[totalTimesteps];
	...
	for (int timestepCounter = 0; timestepCounter < totalTimesteps; ++timestepCounter) {
		...
		// timesteps[timestepCounter] = timestep;
	...
	for (int timestepCounter = 0; timestepCounter < totalTimesteps; timestepCounter++) {
		...
		for (std::vector<int>::iterator aIt = aIndx.begin(); aIt != aIndx.end(); ++aIt) {
			...
			ax = timestepsData[timestepCounter][3 * a];
			ay = timestepsData[timestepCounter][(3 * a) + 1];
			az = timestepsData[timestepCounter][(3 * a) + 2];
			...
			for (std::vector<int>::iterator bIt = bIndx.begin(); bIt != bIndx.end(); ++bIt) {
				...
				bx = timestepsData[timestepCounter][3 * b];
				by = timestepsData[timestepCounter][(3 * b) + 1];
				bz = timestepsData[timestepCounter][(3 * b) + 2];
		...
		delete[] timestepsData[timestepCounter];
	...
	std::cout << "Time to perform calcualtions: " << time << " s" << std::endl;
